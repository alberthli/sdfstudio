...
from sdfstudio.field_components.encodings import SDFEncoding
...
from sdfstudio.fields.vanilla_sdf_field import SDFField
...
class VanillaModelConfig(ModelConfig):
    ...
    _target: Type = field(default_factory=lambda: SDFModel)
    ...
    temporal_distortion_params: Dict[str, Any] = to_immutable_dict({"kind": TemporalDistortionKind.DSDF})
    """Parameters to instantiate temporal distortion with"""


class SDFModel(Model):
    """Vanilla SDF model

    Args:
        config: Basic SDF configuration to instantiate model
    """
    ...
    def populate_modules(self):
        ...
        # fields
        position_encoding = SDFEncoding(
            in_dim=3, num_frequencies=10, min_freq_exp=0.0, max_freq_exp=8.0, include_input=True
        )
        direction_encoding = SDFEncoding(
            in_dim=3, num_frequencies=4, min_freq_exp=0.0, max_freq_exp=4.0, include_input=True
        )

        self.field_coarse = SDFField(
            position_encoding=position_encoding,
            direction_encoding=direction_encoding,
        )

        self.field_fine = SDFField(
            position_encoding=position_encoding,
            direction_encoding=direction_encoding,
        )
