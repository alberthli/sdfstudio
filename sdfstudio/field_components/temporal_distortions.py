...
from sdfstudio.field_components.encodings import Encoding, SDFEncoding
...
class TemporalDistortionKind(Enum):
    ...
    def to_temporal_distortion(self, config: Dict[str, Any]) -> TemporalDistortion:
        ...
        if self == TemporalDistortionKind.DSDF:
            return DSDFDistortion(**config)
        raise NotImplementedError(f"Unknown temporal distortion kind {self}")


class DSDFDistortion(TemporalDistortion):
    ...
    def __init__(
        self,
        position_encoding: Encoding = SDFEncoding(
            in_dim=3, num_frequencies=10, min_freq_exp=0.0, max_freq_exp=8.0, include_input=True
        ),
        temporal_encoding: Encoding = SDFEncoding(
