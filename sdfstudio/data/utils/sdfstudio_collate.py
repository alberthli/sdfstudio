...
def sdfstudio_collate(
    ...
    from default_collate to sdfstudio_collate, and the addition of the sdfstudio_collate_err_msg_format variable.
    ...
    Here is the general input type (based on the type of the element within the batch) to output type mapping:
        ...
        * `Mapping[K, V_i]` -> `Mapping[K, sdfstudio_collate([V_1, V_2, ...])]`
        * `NamedTuple[V1_i, V2_i, ...]` -> `NamedTuple[sdfstudio_collate([V1_1, V1_2, ...]),
          sdfstudio_collate([V2_1, V2_2, ...]), ...]`
        * `Sequence[V1_i, V2_i, ...]` -> `Sequence[sdfstudio_collate([V1_1, V1_2, ...]),
          sdfstudio_collate([V2_1, V2_2, ...]), ...]`
    ...
    Examples:
        ...
        >>> sdfstudio_collate([0, 1, 2, 3])
        tensor([0, 1, 2, 3])
        >>> # Example with a batch of `str`s:
        >>> sdfstudio_collate(['a', 'b', 'c'])
        ['a', 'b', 'c']
        >>> # Example with `Map` inside the batch:
        >>> sdfstudio_collate([{'A': 0, 'B': 1}, {'A': 100, 'B': 100}])
        {'A': tensor([  0, 100]), 'B': tensor([  1, 100])}
        >>> # Example with `NamedTuple` inside the batch:
        >>> Point = namedtuple('Point', ['x', 'y'])
        >>> sdfstudio_collate([Point(0, 0), Point(1, 1)])
    ...
    if isinstance(elem, torch.Tensor):  # pylint: disable=no-else-return
        ...
        if elem_type.__name__ == "ndarray" or elem_type.__name__ == "memmap":
            ...
            if np_str_obj_array_pattern.search(elem.dtype.str) is not None:
                raise TypeError(SDFSTUDIO_COLLATE_ERR_MSG_FORMAT.format(elem.dtype))

            return sdfstudio_collate([torch.as_tensor(b) for b in batch], extra_mappings=extra_mappings)
        ...
        try:
            return elem_type(
                {key: sdfstudio_collate([d[key] for d in batch], extra_mappings=extra_mappings) for key in elem}
            )
        except TypeError:
            # The mapping type may not support `__init__(iterable)`.
            return {key: sdfstudio_collate([d[key] for d in batch], extra_mappings=extra_mappings) for key in elem}
    elif isinstance(elem, tuple) and hasattr(elem, "_fields"):  # namedtuple
        return elem_type(*(sdfstudio_collate(samples, extra_mappings=extra_mappings) for samples in zip(*batch)))
        ...
        if isinstance(elem, tuple):
            return [
                sdfstudio_collate(samples, extra_mappings=extra_mappings) for samples in transposed
            ]  # Backwards compatibility.
        else:
            try:
                return elem_type([sdfstudio_collate(samples, extra_mappings=extra_mappings) for samples in transposed])
            except TypeError:
                # The sequence type may not support `__init__(iterable)` (e.g., `range`).
                return [sdfstudio_collate(samples, extra_mappings=extra_mappings) for samples in transposed]

    # SdfStudio types supported below
    ...
    raise TypeError(SDFSTUDIO_COLLATE_ERR_MSG_FORMAT.format(elem_type))
