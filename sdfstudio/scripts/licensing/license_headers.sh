...
check_failed=false
added_headers=false
for i in $(find sdfstudio/ -name '*.py');
do
  ...
  then
    if [ "$check" = true ];
      then
        ...
        cat sdfstudio/scripts/licensing/copyright.txt $i >$i.new && mv $i.new $i
...
if [ "$check_failed" = true ];
  then
    echo "Run '.sdfstudio/scripts/licensing/license_headers.sh to add missing headers.'"
    exit 1
fi
